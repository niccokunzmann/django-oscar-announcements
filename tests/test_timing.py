...
delete_expired_announcements = Announcement.delete_expired_announcements
...
def timing_user(db):
    user, _ = User.objects.get_or_create(
        username="timing_user", defaults={"is_active": True}
    )
    return user
...
def timing_creator(db):
    creator, _ = User.objects.get_or_create(
        username="timing_admin", defaults={"is_staff": True}
    )
    return creator
...
def _make(creator, **kwargs):
    defaults = dict(
        content="Timing test", visibility="registered", level=Announcement.INFO
    )
...
class TestPublishEnd:
    ...
    def test_end_of_day_still_shown_during_day(self, timing_user, timing_creator):
        ...
        eod = datetime.datetime(
            end_date.year, end_date.month, end_date.day, 23, 59, 59, tzinfo=_UTC
        )
        ann = _make(
            timing_creator,
            publish_start=NOON - datetime.timedelta(hours=12),
            publish_end=eod,
        )

        morning = datetime.datetime(
            end_date.year, end_date.month, end_date.day, 8, 0, 0, tzinfo=_UTC
        )
...
class TestCombinedStates:
    ...
    def test_wrong_audience_hidden_regardless_of_timing(
        self, timing_user, timing_creator
    ):
        ann = _make(
            timing_creator,
            publish_start=NOON - datetime.timedelta(hours=1),
...
class TestDeleteExpiredAnnouncements:
    ...
    def test_end_of_day_not_deleted_during_day(self, timing_creator):
        ...
        morning = datetime.datetime(
            end_date.year, end_date.month, end_date.day, 6, 0, 0, tzinfo=_UTC
        )
        with _patch_now(morning):
            delete_expired_announcements()
        assert Announcement.objects.filter(pk=ann.pk).exists()
